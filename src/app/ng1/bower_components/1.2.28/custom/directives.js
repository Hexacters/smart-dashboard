(function () {
	...
	angular.module(appCon.appName).controller("vmActionController", [ "$injector", "$element", "$scope", "$state", "$rootScope", "$attrs", function($injector, $element, $scope, $state, $rootScope, $attrs){
		        ...
		        $scope.$watch(
					...
					function(newValue) {
						if(newValue.indexOf("{{") < 0){
							...
							console.log(getData.attr('params'));
							loadGetData();
						}
					}
				);
						...
						if(angular.isDefined(config) && angular.isObject(config)){
							if(config.type === "deleteModel" && angular.isDefined(config.dataModel)){
								...
								if(angular.isDefined(config.reloadTable)){
									...
									$state.go(config.reloadState, {}, { reload: true });
								}
								...
								if(angular.isDefined(config.reloadState)) {
						        	$state.go(config.reloadState, {}, { reload: true });
								}
							}
						}
	                        ...
	                        $injector.get(serviceName)[operationName]($scope.data).then(
	                        	...
	                        	function(result) { 
	                        		if (result.data.status === 'success') {
	                        			...
	                        			//authService.setUser(result.data.successData.userProfile);
	                        			authService.requestCurrentUser(true);
	...
	angular.module(appCon.appName).directive('fileUpload', ['FileUploader', function($injector, $rootScope, $state, $parse, $window, $cookieStore, $http) {
	    ...
	    return {
	        ...
	        controller: function($scope, FileUploader, $element, $attrs, $parse, $window, $cookieStore, $http) {
	            ...
	            $scope.$watch('filesList', function() {	            	
                    if (angular.isDefined($scope.filesList)) {
                    	var uploadedId = [];
    	            	console.log(angular.isArray($scope.filesList));
	            ...
	            uploader.onWhenAddingFileFailed = function(item /*{File|FileLikeObject}*/ , filter, options) {
	                ...
	                switch (filterName) {
	                    case 'typeFilter':
	                        $scope.fileError = "Invalid File Format";
	                        break;
	                    case 'sizeFilter':
	                        $scope.fileError = "File size must be less than " + allowedFileSize + "MB";
	                        break;
	                    case 'customFilter':
	                        $scope.fileError = "Reached maximum files to upload";
	                        break;
	                    default:
	                        $scope.fileError = "";
	                }
	            ...
	            var uploaderScopeGetter = $parse($attrs.parentScopeLevel+".data."+$attrs.parentRootNode+"."+$attrs.parentDataField);
	        ...
	        link: function(scope, element) {
	            ...
	            scope.render = function(data) {
	            	...
	            	var line = d3.svg.line()
	            	    .interpolate("linear")
	            	    .x(function(d) { return x(d.chartKey) })
	            	...
	            	var svg = d3.select(element[0]).append("svg")
	            		  ...
	            		  if(yAxisFieldsArray.indexOf(name) != -1){
	            			return {
	            			  ...
	            			  values: chartData.map(function(d) {
	            				return {chartKey: d[xAxisField], chartValue: +d[name]};
	            			  })
	            			};
	            		}
	            	  });
	            	  ...
	            	  if (scope.xaxisparser=='date'){
	            		  x.domain(d3.extent(chartData, function(d) { return d.chartKey; }));
	            	  }else if (scope.xaxisparser=='number'){
	            		  x.domain([
	                	    d3.min(chartData, function(d) { return Number(d.chartKey); }),
	                	    d3.max(chartData, function(d) { return Number(d.chartKey); })
	                	  ]);
	            	  ...
	            	  var minY = d3.min(yAxisFields, function (kv) { return d3.min(kv.values, function (d) { return d.chartValue; }) });
	            	  var maxY = d3.max(yAxisFields, function (kv) { return d3.max(kv.values, function (d) { return d.chartValue; }) });
	            	  ...
	            	  if(showLegend == true || showLegend == 'true'){
		            	/*  var legend = svg.selectAll('g')
		                  ...
		                  .attr('y', function(d, i){  return height + 35;})
		                  .attr('width', 10)
		                  .attr('height', 10)
		                  ...
		                  .attr('y', function(d, i){ return height + 44;})
	...
	**/
	...
	angular.module(appCon.appName).directive("slick", function($injector, $timeout) {
		  return {
		    restrict: "AEC",
		    scope: {
		      initOnload: "@",
