...
function Calendar_constructor(element, overrides) {
	...
	function getAndRenderEvents() {
		if (!options.lazyFetching || isFetchNeeded(currentView.intervalStart, currentView.intervalEnd)) {
			fetchAndRenderEvents();
		}
		else {
			renderEvents();
	...
	function fetchAndRenderEvents() {
		fetchEvents(currentView.intervalStart, currentView.intervalEnd);
...
function Header(calendar, options) {
	...
	function updateTitle(text) {
		...
		var element =jQuery([]);
		var calTitleMonth = calendar.getDate().month();
		var calTitleElement = '';
		var monthsArray = options.monthNames;
		if (monthsArray && monthsArray.length > 0) {
			calTitleElement = calTitleElement +   '<div style="float:left;margin-left:2px;"><select class="form-control fc-vmtitle-select">';
			for (var i = 0; i < monthsArray.length; i++){
				if (i==calTitleMonth){
					calTitleElement = calTitleElement +  '<option value="' + i + '" selected>';
				}else{
					calTitleElement = calTitleElement +  '<option value="' + i + '">';
				}
				calTitleElement = calTitleElement +  monthsArray[i];
				calTitleElement = calTitleElement +  '</option>';
			}
			calTitleElement = calTitleElement +  '</select></div>';
        }
		var calTitleYear = calendar.getDate().year();
		calTitleElement = calTitleElement +  '<div style="float:left;margin-left:5px;"><select class="form-control fc-vmtitle-select">';
		for (var i = calTitleYear-10; i < calTitleYear+10; i++) {
			if (i==calTitleYear){
				calTitleElement = calTitleElement + '<option value="' + i + '" selected>';
			}else{
				calTitleElement = calTitleElement + '<option value="' + i + '">';
			}
			calTitleElement = calTitleElement + i;
			calTitleElement = calTitleElement + '</option>';
		}
        calTitleElement = calTitleElement + '</select></div>';
		el.find('div.fc-center').html(calTitleElement );
		var vmtitleContentElement = el.find('div.fc-center').find('select.fc-vmtitle-select');
		if (vmtitleContentElement.length) {
			var monthContentElement = vmtitleContentElement[0];
			var yearContentElement = vmtitleContentElement[1];
			$(monthContentElement).change(function() {
				
				monthValue = monthContentElement.value;
				yearValue = yearContentElement.value;
				monthValue = options.monthNamesShort[monthValue];
				calendar.gotoDate(yearValue+'/'+monthValue+'/01');
			});
			$(yearContentElement).change(function() {
				monthValue = monthContentElement.value;
				yearValue = yearContentElement.value;
				calendar.gotoDate(yearValue+'/'+monthValue+'/01');
			});
